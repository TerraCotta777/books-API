...
module.exports = (req, res) => {
  ...
  const permisbleKeys =
    "name, author, isFavorite, publishYear, publishHouse, pagesNumber, genres, originalLanguage".split(
      ", "
    );

  const book = db.get("books").find({ id }).value();
  const book_expanded = db.get("books_expanded").find({ id }).value();
  if (!book || !book_expanded)
    return utils.error(res, 404, "cannot find book with this id");
  ...
  if (name && typeof name !== "string")
    return utils.error(res, 400, "name attribute should be type `string`");
  if (author && typeof author !== "string")
    return utils.error(res, 400, "author attribute should be type `string`");
  if (isFavorite && typeof isFavorite !== "boolean")
    return utils.error(
      res,
      400,
      "isFavorite attribute should be type `boolean`"
    );
  if (publishYear && typeof publishYear !== "number")
    return utils.error(
      res,
      400,
      "publishYear attribute should be type `number`"
    );
  if (publishHouse && typeof publishHouse !== "string")
    return utils.error(
      res,
      400,
      "publishHouse attribute should be type `string`"
    );
  if (pagesNumber && typeof pagesNumber !== "number")
    return utils.error(
      res,
      400,
      "pagesNumber attribute should be type `number`"
    );
  if (genres && !Array.isArray(genres))
    return utils.error(res, 400, "genres attribute should be type `array`");
  if (originalLanguage && typeof originalLanguage !== "string")
    return utils.error(
      res,
      400,
      "originalLanguage attribute should be type `string`"
    );
